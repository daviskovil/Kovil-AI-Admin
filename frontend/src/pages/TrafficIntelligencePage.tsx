        ...
        >
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${statusDot(gscAgent.status)}`} />
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{gscAgent.code}</span>
              <span className="text-[9px] font-bold text-orange-500 uppercase tracking-wider">· Flagship</span>
            </div>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${scoreBadge(gscAgent.score)}`}>
              {scoreLabel(gscAgent.score, gscAgent.status)}
            </span>
          </div>
          <h3 className="text-sm font-semibold text-gray-900 group-hover:text-orange-500 transition-colors leading-snug mb-1">
            {gscAgent.label}
          </h3>
          <p className="text-[10px] text-gray-400 mb-4">{gscAgent.schedule}</p>
          <div className="flex items-center gap-3 mb-3">
            <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
              <div
                className={`h-2 rounded-full transition-all ${scoreColor(gscAgent.score, 'bg')}`}
                style={{ width: `${gscAgent.score}%` }}
              />
            </div>
            <span className={`text-sm font-bold w-8 text-right ${scoreColor(gscAgent.score, 'text')}`}>
              {gscAgent.score}
            </span>
          </div>
          <p className="text-[11px] text-gray-500 leading-relaxed line-clamp-2">{gscAgent.topFinding}</p>
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
            <p className="text-[10px] text-gray-300">
              {gscAgent.lastScanned ? `Scanned ${gscAgent.lastScanned}` : 'Never scanned'}
            </p>
            <button
